...
namespace MyMod
{
    public class MyWorld : ModWorld
    {
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
        {
            ...
            if (graniteIndex != -1)
            {
                tasks.Insert(graniteIndex + 1, new PassLegacy("Deadmans Cave", GenerateDeadmansCave));
            }
        }

        private void GenerateDeadmansCave(GenerationProgress progress)
        {
            // Set the progress message
            progress.Message = "Creating Deadmans Cave";
            ...
            if (chestIndex >= 0)
            {
                ...
                {
                    int j = WorldGen.genRand.Next(i, loot.Length);
                    Item temp = loot[i];
                    loot[i] = loot[j];
                    loot[j] = temp;
                }
                ...
                for (int i = 0; i < loot.Length; i++)
                {
                    Main.chest[chestIndex].item[i] = loot[i];
                }
            }
        }
    }
